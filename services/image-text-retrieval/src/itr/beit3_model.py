...
class Preprocess():
    ...
    def preprocess(self, input: Union[str, Image.Image]):
        if isinstance(input, str):
            ...
            return torch.LongTensor(tokens + [self.pad_token_id] * (self.max_len - num_tokens)).unsqueeze(0), torch.Tensor(padding_mask).unsqueeze(0), num_tokens
...
class Beit3Model:
    def __init__(self, model_name: str = 'beit3_base_patch16_384_retrieval', model_path: str = "./beit3_model/beit3_base_patch16_384_f30k_retrieval.pth", device: str = "cuda"):
    ...
    def _load_model(self, model_name, model_path, device: str = "cpu"):
        ...
        self.preprocessor = Preprocess(XLMRobertaTokenizer("./beit3_model/beit3.spm"))
    ...
    def get_embedding(self, input: Union[str, Image.Image]):
        if isinstance(input, str):
            ...
            _, vector = self.model(text_description=token_ids, padding_mask=padding_mask, only_infer=True)
