...
import numpy as np
import torch
from evaluation.metrics import (compute_metrics, tensor_text_to_video_metrics,
...
def _run_on_single_gpu(
    ...
    for idx1, b1 in enumerate(batch_list_t):
        ...
        for idx2, b2 in enumerate(batch_list_v):
            ...
            b1b2_logits, *_tmp = model.get_inference_logits(
...
def eval_epoch(model, test_dataloader, device, n_gpu, logger):
    ...
    if (
        ...
        cut_off_points_ = (
            test_dataloader.dataset.cut_off_points
        )  # used to tag the label when calculate the metric
        sentence_num_ = (
            test_dataloader.dataset.sentence_num
    ...
    if multi_sentence_:
        ...
        logger.warning(f"sentence num: {sentence_num_}, video num: {video_num_}")
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            print(f"{bid}/{len(test_dataloader)}\r", end="")
        ...
        if n_gpu > 1:
            ...
            for dev_id in device_ids:
                ...
                if dev_id == 0:
                    ...
                    devc = torch.device(f"cuda:{str(dev_id)}")
                    ...
                    if isinstance(batch_sequence_output_list[s_], tuple):
                        # for multi_output
                        devc_batch_list = [
                            (b[0].to(devc), b[1].to(devc))
                            for b in batch_sequence_output_list[s_:e_]
                        ]
                    else:
                        # for single_output
...
def logging_rank(sim_matrix, multi_sentence_, cut_off_points_, logger):
    ...
    if multi_sentence_:
        ...
        logger.info(
            f"before reshape, sim matrix size: {sim_matrix.shape[0]} x {sim_matrix.shape[1]}"
        )
        ...
        logger.info(f"\t Length-T: {len(sim_matrix)}, Length-V:{len(sim_matrix[0])}")
