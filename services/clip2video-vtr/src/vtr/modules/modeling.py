...
class CLIP2VideoPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # initialize the model with other vtr.modules
...
class CLIP2Video(CLIP2VideoPreTrainedModel):
    ...
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        if self.temporal_proj == "sigmoid_mlp":
            ...
            self.frame2t_attention = TransformerClip(
                width=transformer_width,
                layers=1,
                heads=transformer_heads,
            )
        ...
        if self.center_type == "TAB":
            ...
            if self.center_proj == "TAB" or self.center_proj == "TAB_TDB":
                self.actionClip = TransformerClip(
                    width=transformer_width,
                    layers=1,
                    heads=transformer_heads,
                )
    ...
    def get_extra_TAB_embedding(self, embedding_out, attention_mask):
        ...
        """
        large_position_d = torch.arange(
            start=0,
            end=embedding_out.size()[1],
            step=2,
            dtype=torch.long,
        ...
        if self.center_proj == "TAB" or self.center_proj == "TAB_TDB":
            ...
            if self.center_proj == "TAB_TDB":
                ...
                large_embedding_out = (
                    large_embedding_out + TAB_position_embedding
                )  # batch_size * 12 * 512

            extended_video_mask = (
                1.0 - large_attention_mask_span.unsqueeze(1)
            ) * -1000000.0  # batch_size * 1* 12
            extended_video_mask = extended_video_mask.expand(
                -1, large_attention_mask_span.size(1), -1
            )  # batch_size * 12 * 12

            # adopt 1-layer temporal transformer to encode representation
            large_embedding_out = large_embedding_out.permute(
                1, 0, 2
            )  # NLD -> LND # 12 * batch_size * 512
            large_embedding_out = self.actionClip(
                large_embedding_out, extended_video_mask
            )  # 12 * batch_size * 512
            large_embedding_out = large_embedding_out.permute(
                1, 0, 2
            ...
            if self.center_proj == "TAB_TDB":
                frame_position_id = torch.arange(
                    start=0,
                    end=large_embedding_out.size()[1],
                    step=2,
                    dtype=torch.long,
    ...
    def get_TAB_embedding(self, embedding_out, attention_mask, type="default"):
        ...
        """
        if type == "visual":
            embedding_out, attention_mask = self.get_extra_TAB_embedding(
    ...
    def calc_TAB_loss(
        ...
        cluster_sequence_output = self.get_TAB_embedding(
            sequence_hidden_output, attention_mask_un, type="sequence"
        )
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.center_type == "TAB":
                ...
                sim_loss = self.calc_loss(
                    sequence_output, visual_output, attention_mask, video_mask
                )
                loss += sim_loss * self.center_weight

                # calculate TAB loss
                TAB_loss = self.calc_TAB_loss(
                    sequence_hidden_output, visual_output, attention_mask, video_mask
                )
    ...
    def temporal_difference_block(self, visual_output, video_mask):
        ...
        # batch size * 11 * 512
        dif_visual_output = (
            visual_output[:, 1:seq_length, :] - visual_output[:, 0 : seq_length - 1, :]
        )
        if self.temporal_proj == "sigmoid_mlp":
            ...
            # batch_size * 11 * 11
            extend_trans_video_mask = extend_trans_video_mask.expand(
                -1, trans_video_mask.size(1), -1
            )
        ...
        frame_position_embeddings_middle = torch.cat(
        ...
        seq2_indices = torch.arange(
            start=seq_length, end=2 * seq_length - 1, step=1, dtype=torch.long
        )
    ...
    def _similarity_TDB(
        ...
        visual_output = (
            visual_output + frame_position_embeddings + type_embedding
        )  # batch_size * 12 * 512
        extended_video_mask = (
            1.0 - temporal_video_mask.unsqueeze(1)
        ) * -1000000.0  # batch_size * 1* 12
        extended_video_mask = extended_video_mask.expand(
    ...
    def get_similarity_logits(
        self, sequence_output, visual_output, attention_mask, video_mask, shaped=False
    ):
        ...
        if self.sim_type == "seqTransf" and self.temporal_type == "TDB":
            ...
            retrieve_logits = self._similarity_TDB(
                sequence_output, visual_output, attention_mask, video_mask
            )
        else:
            ...
            retrieve_logits = self._similarity(
                sequence_output,
                visual_output,
                attention_mask,
                video_mask,
    ...
    def get_inference_logits(
        ...
        if self.center_type == "TAB":
            ...
            retrieve_logits = retrieve_logits * self.center_weight + sim_matrix * (
                1 - self.center_weight
            )

        return retrieve_logits, contrastive_direction

    def extract_global_text_features(self, sequence_output, attention_mask):
        if self.center_type == "TAB":
    ...
    def extract_local_text_features(self, sequence_output, attention_mask):
        if self.center_type == "TAB":
            _, sequence_hidden_output = sequence_output
            ...
            return cluster_sequence_output
        else:
            return torch.zeros_like(sequence_output)

    def extract_global_visual_features(self, visual_output, video_mask):
        ...
        if self.sim_type == "seqTransf" and self.temporal_type == "TDB":
            ...
            # adopting mean pooling or use temporal transformer to aggregate the video representation
            assert self.sim_type in ["meanP", "seqTransf"]
            if self.sim_type == "meanP":
                # Default: Parameter-free type
                pass
            elif self.sim_type == "seqTransf":
                # Sequential type: Transformer Encoder
                visual_output_original = visual_output  # batch_size * 12 * 512
                seq_length = visual_output.size(1)  # 12
                # obtain positional embedding
    ...
    def extract_local_visual_features(self, visual_output, video_mask):
        if self.center_type == "TAB":
            # calculate the aligned video representation
            video_mask_un = video_mask.to(dtype=torch.float).unsqueeze(-1)
            video_mask_un = video_mask_un.unsqueeze(-1)
            ...
            return cluster_visual_output
        else:
            return torch.zeros_like(visual_output)

    def extract_text_features(self, sequence_output, attention_mask, shaped=False):
        if shaped is False:
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
    ...
    def extract_visual_features(self, visual_output, video_mask, shaped=False):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
    ...
    def get_sim_matrix(self, text_features, visual_features):
        # calculate the similarity
        logit_scale = self.clip.logit_scale.exp()
        sim_matrix = logit_scale * torch.matmul(text_features, visual_features.t())
